# streamlit_app.py - Laboratory Reagent Inventory (All Errors Fixed)
...
def init_db():
    ...
    # Default accounts - CHANGE PASSWORDS ASAP!
    ...
    with st.form("login_form"):
        ...
        if submitted:
            ...
            if result:
                ...
                st.success(f"Welcome, {username} ({result[0]})")
                st.rerun()
            else:
...
with tab2:
    ...
    with st.form("add_form"):
        ...
        name = col1.text_input("Name*", key="add_name")
        cas = col1.text_input("CAS Number")
        supplier = col2.text_input("Supplier")
        location = col2.text_input("Location*", key="add_location")
        quantity = col1.number_input("Quantity*", min_value=0.0, step=0.1)
        unit = col1.selectbox("Unit", ["g", "mg", "ml", "L", "bottles", "vials"])
...
with tab3:
    ...
    if reagents_df.empty:
        st.info("No reagents to log usage for yet.")
    else:
        ...
        if st.button("Record Usage"):
            current = reagents_df.loc[reagents_df['id'] == reagent_id, 'quantity'].values[0]
            if qty_used > current:
...
with tab4:
    ...
    if reagents_df.empty:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Generate QR Label")
            ...
            app_url = st.text_input("Your App URL", value="https://your-app-name.streamlit.app",
            ...
            st.download_button("Download QR Label", byte_im,
...
st.caption("Laboratory Reagent Inventory • Built with Streamlit • Data may reset on Cloud reboots")
