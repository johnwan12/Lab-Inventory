...
with tab2:
    ...
    # Dynamic form key for auto-clear after submit
    ...
    photo = st.camera_input("Take photo of label") or st.file_uploader("Or upload photo", type=["jpg","png","jpeg"])
    
    if photo:
        st.image(photo, caption="Label photo", width=400)
        ...
        with st.spinner("OCR processing..."):
            try:
                ...
                st.info("Copy useful info (name, CAS, catalog #, etc.) to the form above.")
    ...
    if reagents_df.empty:
        ...
        reagent_id = st.selectbox(
            ...
            format_func=lambda x: f"{reagents_df.loc[reagents_df['id']==x, 'name'].values[0]} "
                                 f"({reagents_df.loc[reagents_df['id']==x, 'quantity'].values[0]:.2f} left)"
        )
        col1, col2 = st.columns(2)
...
with tab4:
    ...
    if reagents_df.empty:
        ...
        with col1:
            ...
            selected_id = st.selectbox("Select", reagents_df['id'],
            ...
            st.image(byte_im, caption="QR Label")
            st.download_button("Download", byte_im, f"QR_{row['name']}_{selected_id}.png", "image/png")
...
with tab5:
    if st.session_state.role != "admin":
        ...
        col1.metric("Total", len(reagents_df))
